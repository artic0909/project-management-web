...
<main class="page-area" id="pageArea">
    <div class="page" id="page-meetings-dev">
        ...
        <div class="dash-card" style="margin-bottom:24px;">
            <form action="{{ route('developer.meetings.index') }}" method="GET" id="filterForm">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($meeting->meeting_type == 'lead' && $meeting->lead)
                                            <span class="m-link active" style="font-size:12px;"><i class="bi bi-person-circle"></i> {{ $meeting->lead->company }}</span>
                                        @elseif($meeting->meeting_type == 'order' && $meeting->order)
                                            <span class="m-link active" style="font-size:12px;"><i class="bi bi-bag-check"></i> Order #{{ $meeting->order->id }}</span>
                                        @elseif($meeting->meeting_type == 'project' && $meeting->project)
                                            <a href="{{ route('developer.projects.show', $meeting->project_id) }}" class="m-link active" style="font-size:12px;"><i class="bi bi-kanban"></i> {{ $meeting->project->name }}</a>
                                        @endif
                                    </td>
                                    <td>
                                        ...
                                        <div class="participant-stack">
                                            ...
                                            @foreach($developers->whereIn('id', $d_ids ?? []) as $d)
                                                <div class="p-item">
                                                    ...
                                                    @if($d->id == auth()->guard('developer')->id()) <span style="font-size:9px; color:var(--accent); font-weight:800;">(You)</span> @endif
...
<style>
    ...
    @media (max-width: 1100px) {
        .m-meta-grid { grid-template-columns: 1fr; gap: 10px; }
    }
