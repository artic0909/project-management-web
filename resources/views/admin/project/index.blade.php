...
<main class="page-area" id="pageArea">
    <div class="page" id="page-projects">
        ...
        {{-- ── KPI CARDS ── --}}
        <div id="statsWrap" class="stats-grid-wrap" style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:22px;">
                            ...
                            <tr>
                                    ...
                                    <div class="ls">
                                        @if(is_array($project->phones) && count($project->phones) > 0)
                                            @php
                                                ...
                                                $num = is_array($firstPhone) ? ($firstPhone['number'] ?? $firstPhone['num'] ?? 'N/A') : $firstPhone;
                                                $idx = is_array($firstPhone) ? ($firstPhone['code'] ?? $firstPhone['code_idx'] ?? null) : null;
                                                $phoneCode = ($idx !== null && isset($codes[$idx])) ? $codes[$idx] : '';
                                ...
                                <td>
                                    <div class="row-actions">
                                        @php
                                            $codes = [0=>'+93',1=>'+355',2=>'+213',3=>'+376',4=>'+244',5=>'+54',6=>'+61',7=>'+43',8=>'+880',9=>'+32',10=>'+55',11=>'+1',12=>'+86',13=>'+57',14=>'+45',15=>'+20',16=>'+33',17=>'+49',18=>'+233',19=>'+30',20=>'+91',21=>'+62',22=>'+98',23=>'+964',24=>'+353',25=>'+972',26=>'+39',27=>'+81',28=>'+962',29=>'+254',30=>'+965',31=>'+961',32=>'+60',33=>'+52',34=>'+212',35=>'+977',36=>'+31',37=>'+64',38=>'+234',39=>'+47',40=>'+968',41=>'+92',42=>'+63',43=>'+48',44=>'+351',45=>'+974',46=>'+7',47=>'+966',48=>'+65',49=>'+27',50=>'+34',51=>'+94',52=>'+46',53=>'+41',54=>'+886',55=>'+66',56=>'+90',57=>'+971',58=>'+44',59=>'+1',60=>'+84',61=>'+260',62=>'+263'];
                                            $phoneList = is_array($project->phones) ? $project->phones : (json_decode($project->phones, true) ?? []);
                                            $emailList = is_array($project->emails) ? $project->emails : (json_decode($project->emails, true) ?? []);
                                            $fullPhones = [];
                                            foreach($phoneList as $p) {
                                                $num = is_array($p) ? ($p['number'] ?? $p['num'] ?? '') : $p;
                                                $idx = is_array($p) ? ($p['code'] ?? $p['code_idx'] ?? null) : null;
                                                $fullPhones[] = ($idx !== null && isset($codes[$idx])) ? ($codes[$idx] . $num) : $num;
                                            }
                                            $firstPhone = reset($phoneList);
                                            $wpPhone = '';
                                            if ($firstPhone) {
                                                $firstNum = is_array($firstPhone) ? ($firstPhone['number'] ?? $firstPhone['num'] ?? '') : $firstPhone;
                                                $firstIdx = is_array($firstPhone) ? ($firstPhone['code'] ?? $firstPhone['code_idx'] ?? null) : null;
                                                $codeStr = ($firstIdx !== null && isset($codes[$firstIdx])) ? $codes[$firstIdx] : '';
                                                $cleanCode = preg_replace('/[^0-9]/', '', $codeStr);
                                                $cleanNumber = preg_replace('/[^0-9]/', '', $firstNum);
                                                $wpPhone = $cleanCode . $cleanNumber;
                                            }
                                        @endphp
                                        @if(!empty($wpPhone))
                                        <a href="https://web.whatsapp.com/send?phone={{ $wpPhone }}" target="_blank" rel="noopener noreferrer" class="ra-btn whatsapp" title="WhatsApp Client">
