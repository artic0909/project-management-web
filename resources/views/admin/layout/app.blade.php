                                ...
                                @foreach($todayTimedFollowups as $lead)
                                    ...
                                    <a href="{{ route($guard . '.followup.read_notif', $latestFollowup->id) }}" class="notif-item unread" onclick="this.style.display='none'; let b = document.querySelector('.notif-badge'); if(b){let c=parseInt(b.innerText)-1; if(c<=0)b.remove(); else b.innerText=c;}">
                                        ...
                                        <div class="notif-body">
                                            <strong>{{ $lead->company ?: $lead->contact_person ?: 'Lead #' . $lead->id }}</strong>
                                            @php
                                            ...
                                            @if($displayPhone)
                                                <div style="font-size: 11px; color: var(--t4); margin-top: 2px;">
                                                    <i class="bi bi-telephone"></i> {{ $displayPhone }}
                                                </div>
                                            @endif
                                            <div class="notif-time" style="color:var(--accent); margin-top:2px;">Scheduled at {{ \Carbon\Carbon::parse($latestFollowup->next_schedule_date)->format('h:i A') }}</div>
                                        </div>
                                    </a>
                                @endforeach
                            @endif
