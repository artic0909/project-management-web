...
<script>
    ...
    function openEditOrderNoteModal(id, notes) {
        const modal = document.getElementById('editOrderNoteModal');
        const form = document.getElementById('editOrderNoteForm');
        const textarea = form.querySelector('textarea');
        
        textarea.value = notes;
        form.action = `/admin/order-notes/${id}`;
        modal.style.display = 'flex';
    }
</script>

{{-- Edit Order Note Modal --}}
<div class="modal-backdrop" id="editOrderNoteModal" onclick="closeModal('editOrderNoteModal')">
    <div class="modal-box" onclick="event.stopPropagation()">
        <div class="modal-hd">
            <span>Edit Order Note</span>
            <button class="modal-close" onclick="closeModal('editOrderNoteModal')"><i class="bi bi-x-lg"></i></button>
        </div>
        <div class="modal-bd">
            <form id="editOrderNoteForm" method="POST">
                ...
                <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:20px;">
                    <button type="button" class="btn-ghost" onclick="closeModal('editOrderNoteModal')">Cancel</button>
