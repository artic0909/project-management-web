...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $totalLeads = \App\Models\Lead::where(function($q) use ($saleId, $saleType) {
        ...
        $totalOrders = \App\Models\Order::where(function($q) use ($saleId, $saleType) {
            $q->where('created_by', $saleId)->where('created_by_type', $saleType);
        })->orWhereHas('assignments', function($q) use ($saleId) {
            $q->where('assigned_to', $saleId);
        })->count();

        $orderIds = \App\Models\Order::where(function($q) use ($saleId, $saleType) {
            $q->where('created_by', $saleId)->where('created_by_type', $saleType);
        })->orWhereHas('assignments', function($q) use ($saleId) {
            $q->where('assigned_to', $saleId);
        })->pluck('id');
